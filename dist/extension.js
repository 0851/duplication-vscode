...
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let serverModule = context.asAbsolutePath(path.resolve('dist', 'server.js'));
        ...
        client.onReady().then(() => {
            client.onNotification(config_1.LoadingCommand, () => {
                loading++;
                updateStatusBar();
            });
            client.onNotification(config_1.LoadingHideCommand, (res) => {
                loading--;
                updateStatusBar(res);
            });
            client.onNotification(config_1.ShowQuickPickCommand, (res) => {
                if (vscode_1.workspace.workspaceFolders) {
                    quickpick_1.quickPick(res, vscode_1.workspace.workspaceFolders[0].uri.path);
                    updateStatusBar(res);
                }
            });
        });
...
function quickPick(diff, root) {
    return __awaiter(this, void 0, void 0, function* () {
        let picks = diff.map((item) => {
            return Object.assign({ label: `${removeroot(item.a.filename, root)}:${item.a.start.line} <==> ${removeroot(item.b.filename, root)}:${item.b.start.line}` }, item);
        });
